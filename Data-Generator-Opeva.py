...
ROTINA_CHANGE = 0.05 # Hipotese de ter uma trip time diferente na semana
...
charger_ex = [
    ...
    ["12",["charger_14_1"],75,17],
    ["13",["charger_14_1"],80,18],
...
semana_dia = [ # Prob ; Min Left ; Max Left 
    ("Normal1", 0.90,7,9), # 7-8  
    ("Cedo", 0.05,5,6), # 5-6  
    ("tarde", 0.05,10,12), # 9-12   
]
semana_noite = [ # Prob ; Min Arrive ; Max Arrive 
    ("Normal1", 0.77,17,19), # 17-18
    ("tarde", 0.20,20,22), # 20-22  
    ("cedo", 0.03,14,16), # 14-16
]
...
def gen_traffic(weekend,time_taken):
    ...
    time_percentage_plus = random.randint(int(gen[2] * 100), int(gen[3] * 100))
...
def convert_format(arr,index):
    ...
    separator = ","
    with open("C:\\Users\\steam\\Desktop\\Bolsa\\Opeva\\EV"+charger_ex[int(index)][0]+'.csv', 'w') as file:
...
for x in range(len(charger_ex)):
    ...
    filename = 'C:\\Users\\steam\\Desktop\\Bolsa\\Opeva\\EV-'+charger_ex[x][0]+'.csv'
